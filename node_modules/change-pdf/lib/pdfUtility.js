...
setQuality = quality => {
  ...
  if (QUALITY.indexOf(quality) == -1)
    throw Error(`Quality can contain ${QUALITY}`)
  return quality
}

pdfManipulation.compressPdf = (pdfPath, targetPath, quality) => {
  let setQuality = QUALITY.indexOf(quality) !== -1 ? quality : 'default'
  let compressCommand = `gswin64c -sDEVICE=pdfwrite -dCompatibilityLevel=1.4 -dPDFSETTINGS=/${setQuality} -dDownsampleColorImages=true -dColorImageResolution=150 -dNOPAUSE -dQUIET -dBATCH -sOutputFile=${targetPath} ${pdfPath}`
...
pdfManipulation.genrateImage = (pdfPath, targetPath, conversionType) => {
  let page = 0
  let pdfDetail = {}
  if (conversionType && conversionType != 'thumb')
    throw Error(`conversionType can contain thumb value only`)
  return new Promise((resolve, reject) => {
    pdfManipulation
      .countPages(pdfPath)
      .then(pdfLength => {
...
imageProcess = (page, pdfDetail, resolve, reject) => {
  ...
  let conversionComaand = pdfDetail.conversionType
    ? tumbnailCommand
    : imageCommand
